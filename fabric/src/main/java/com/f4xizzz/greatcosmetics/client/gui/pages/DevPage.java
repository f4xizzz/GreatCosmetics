...
public class DevPage extends WardrobePage {
    ...
    public void render(GuiGraphics c, int mouseX, int mouseY, float delta, int x, int y, int width, int height) {
        ...
        // --- MENU INICIAL ---
        c.drawString(getTextRenderer(), com.f4xizzz.greatcosmetics.config.LangConfig.legacy("devstudio.header"), x + 16, y + 16, 0xFFFFFF);
        c.drawString(getTextRenderer(), com.f4xizzz.greatcosmetics.config.LangConfig.legacy("devstudio.menu.subtitle"), x + 16, y + 32, 0xFFAAAAAA);
        ...
        for (String line : com.f4xizzz.greatcosmetics.config.LangConfig.legacy("devstudio.menu.help").split("\n")) {
            c.drawCenteredString(getTextRenderer(), line, x + (width / 2), helpY, 0xAAAAAA);
            helpY += 10;
        }
    }

    private void drawMenuButton(GuiGraphics c, String label, int bx, int by, int bw, int bh, int mx, int my) {
        ...
        c.drawString(getTextRenderer(), label, bx + 8, by + 6, 0xFFFFFF);
    }
        ...
        if (keyCode == org.lwjgl.glfw.GLFW.GLFW_KEY_ESCAPE) {
            if (this.activeSubPage != null && this.activeSubPage.hasUnsavedChanges) {
                this.activeSubPage.tryExit(() -> {
                    Minecraft.getInstance().setScreen(null); // Fecha a tela só depois de confirmar
                });
