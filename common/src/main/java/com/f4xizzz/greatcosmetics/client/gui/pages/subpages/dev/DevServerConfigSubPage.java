...
public class DevServerConfigSubPage extends DevSubPage {
    ...
    private static class EditorRow {
        String label; String id; RowType type; EditBox textField;
        ...
        EditorRow(String label, EditBox field) { this.label = label; this.id = label; this.type = RowType.STRING; this.textField = field; }
    ...
    public void render(GuiGraphics c, int mouseX, int mouseY, float delta, int x, int y, int width, int height) {
        ...
        c.drawCenteredString(parent.getTextRenderer(), L("devstudio.serverconfig.title"), x + (width / 2), topY + 2, 0xFFFFFF);
        ...
        for (int i = 0; i < this.rows.size(); i++) {
            ...
            if (row.type == RowType.STRING || row.type == RowType.INT) {
                ...
                c.renderOutline(x + 15, rowY + 12, width - 35, 16, row.toggleValue ? 0xFF00FF00 : 0xFFFF0000);
                c.drawCenteredString(parent.getTextRenderer(), L(row.toggleValue ? "devstudio.common.on" : "devstudio.common.off"), x + (width/2), rowY + 16, 0xFFFFFF);
            }
