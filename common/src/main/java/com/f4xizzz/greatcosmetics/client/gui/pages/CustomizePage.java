...
public class CustomizePage extends WardrobePage {
    ...
    public void render(GuiGraphics c, int mouseX, int mouseY, float delta, int x, int y, int width, int height) {
        ...
        c.drawString(getTextRenderer(), com.f4xizzz.greatcosmetics.config.LangConfig.legacy("wardrobe.customize.header"), x + 16, y + 40, 0xFFFFAA);
    ...
    private void drawToggleButton(GuiGraphics c, String label, boolean isHidden, int bx, int by, int mx, int my) {
        ...
        c.drawString(getTextRenderer(), status, bx + 148 - getTextRenderer().width(status) - 6, by + 6, accentColor);
    }

    @Override
    public boolean mouseClicked(double mx, double my, int button) {
        if (button != 0 || Minecraft.getInstance().player == null) return false;
    ...
    private void toggleSetting(String settingName, boolean currentState, ClientCosmeticCache.PlayerSettings old) {
        ...
        com.f4xizzz.greatcosmetics.platform.GcNet.toServer(new ToggleVisibilityPayload(settingName, newState));
