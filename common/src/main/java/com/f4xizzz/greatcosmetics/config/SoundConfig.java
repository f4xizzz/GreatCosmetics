...
public class SoundConfig {
    ...
    private static final File FILE = new File(Platform.getConfigFolder().toFile(), "GreatCosmetics/sounds.json");
    ...
    public static void loadSounds() {
        if (FILE.exists()) {
            try (FileReader reader = new FileReader(FILE)) {
                ...
                com.f4xizzz.greatcosmetics.GreatCosmeticsCommon.debugLog("SoundConfig: FAILED to read sounds.json — " + e + ". Recreating with defaults.");
                saveSounds();
            }
        } else {
            saveSounds();
        }
        com.f4xizzz.greatcosmetics.GreatCosmeticsCommon.debugLog("SoundConfig: sounds.json loaded.");
    }
