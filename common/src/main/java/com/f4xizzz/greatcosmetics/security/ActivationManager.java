...
public final class ActivationManager {
    ...
    private static final File CONFIG_DIR =
            new File(Platform.getConfigFolder().toFile(), "GreatCosmetics");
    ...
    public static void checkLicenseOnStartup(MinecraftServer server) {
        ...
        if (!server.isDedicatedServer() || Platform.isDevelopmentEnvironment()) {
            state = State.ACTIVE;
        ...
        state = State.INACTIVE;
        com.f4xizzz.greatcosmetics.GreatCosmeticsCommon.LOGGER.warn("[GreatCosmetics] License inactive. Run /gc activation <key>.");
    }
    ...
    public static void tickPeriodicCheck(MinecraftServer server) {
        if (!server.isDedicatedServer() || Platform.isDevelopmentEnvironment()) return;
