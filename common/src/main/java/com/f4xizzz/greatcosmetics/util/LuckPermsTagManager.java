...
public class LuckPermsTagManager {
    ...
    /** Callback disparado quando o LuckPerms recalcula os dados de um jogador conectado (troca de
     *  cargo por fora do mod). O entrypoint server liga isso pra revalidar/re-sincronizar as tags
     *  de grupo — estava chamando GreatCosmetics.validateEquippedGroupTag/autoEquipCurrentGroupTag/
     *  syncPlayerTags direto, mas esses métodos continuam no entrypoint Fabric (Phase 3 move). */
    public static java.util.function.Consumer<ServerPlayer> onGroupDataRecalculated = p -> {};
        ...
        try {
            luckPerms.getEventBus().subscribe(
                    ...
                    event -> server.execute(() -> {
                        ...
                        if (player != null) {
                            onGroupDataRecalculated.accept(player);
                        }
                    })
            );
    ...
    public static String[] getPrefixSuffix(ServerPlayer player) {
        LuckPerms luckPerms = api();
    ...
    public static boolean isInGroup(ServerPlayer player, String groupName) {
        LuckPerms luckPerms = api();
    ...
    public static void applyTag(ServerPlayer player, TagData data) {
        LuckPerms luckPerms = api();
        ...
        if (data.minecraftTag != null && !data.minecraftTag.isBlank()) {
            player.addTag(data.minecraftTag);
        }
    }

    /** Remove exatamente os nodes que {@link #applyTag} teria adicionado pra essa tag. */
    public static void removeTag(ServerPlayer player, TagData data) {
        LuckPerms luckPerms = api();
        ...
        if (data != null && data.minecraftTag != null && !data.minecraftTag.isBlank()) {
            player.removeTag(data.minecraftTag);
        }
    }
