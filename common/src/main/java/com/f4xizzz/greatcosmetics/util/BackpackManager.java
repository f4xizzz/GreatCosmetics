...
public class BackpackManager {
    ...
    public static void handleOpenRequest(ServerPlayer player) {
        ...
        if (openSessions.containsKey(player.getUUID()) && player.containerMenu instanceof ChestMenu) {
            com.f4xizzz.greatcosmetics.GreatCosmeticsCommon.debugLog("BackpackManager: fechando mochila '" + openSessions.get(player.getUUID()) + "' rastreada antes de reabrir/mostrar seletor pra " + player.getName().getString());
        ...
        if (backpackIds.size() == 1) {
            ...
            com.f4xizzz.greatcosmetics.platform.GcNet.toPlayer(player, new ShowBackpackSelectorPayload(backpackIds));
        }
    }

    public static void openSpecificBackpack(ServerPlayer player, String cosmeticId) {
        CosmeticData data = com.f4xizzz.greatcosmetics.config.CosmeticsConfig.getCosmeticById(cosmeticId);
        ...
        com.f4xizzz.greatcosmetics.GreatCosmeticsCommon.debugLog("BackpackManager: openSpecificBackpack(" + cosmeticId + ") player=" + player.getName().getString());
        ...
        if (!isReopen && data.sounds != null && data.sounds.backpackSound != null && !data.sounds.backpackSound.isEmpty()) {
            com.f4xizzz.greatcosmetics.GcServer.playCosmeticSound(player, data.sounds.backpackSound, (float) data.sounds.backpackVolume, (float) data.sounds.backpackPitch);
        }
