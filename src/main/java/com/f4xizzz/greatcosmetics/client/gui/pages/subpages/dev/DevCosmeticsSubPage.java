...
public class DevCosmeticsSubPage extends DevSubPage {
    ...
    private TextFieldWidget openDropdownField = null;
    ...
    private class FloatingPopup {
        ...
        void drawBtn(DrawContext c, String label, int bx, int by, int bw, int bh, int mx, int my, int activeColor) {
            ...
            c.drawCenteredTextWithShadow(parent.getTextRenderer(), label, bx + (bw/2), by + 3, 0xFFFFFF);
        }
        ...
        void render(DrawContext c, int mx, int my, float delta) {
            c.getMatrices().push();
            c.getMatrices().translate(0, 0, 100);

            c.fill(x, y, x + width, y + height, 0xF2111111);
            c.drawBorder(x, y, width, height, 0xFFFFAA00);

            c.fill(x, y, x + width, y + 16, 0xFFFFAA00);
            c.drawTextWithShadow(parent.getTextRenderer(), "§l" + title, x + 5, y + 4, 0xFFFFFF);

            boolean hovClose = mx >= x + width - 16 && mx <= x + width && my >= y && my <= y + 16;
            c.fill(x + width - 16, y, x + width, y + 16, hovClose ? 0xFFFF5555 : 0x00000000);
            c.drawCenteredTextWithShadow(parent.getTextRenderer(), "X", x + width - 8, y + 4, 0xFFFFFF);
                ...
                if (row.type == RowType.GIZMO_MODE) {
                    ...
                    if (row.type == RowType.STRING || row.type == RowType.INT || row.type == RowType.DOUBLE) {
                        ...
                        if (row.suggestions != null && row.textField.isFocused()) {
                            ...
                            openDropdownFiltered = filterSuggestions(row.suggestions.get(), row.textField.getText());
                        }
    ...
    private net.minecraft.registry.RegistryWrapper.WrapperLookup registries() {
        var world = MinecraftClient.getInstance().world;
        return world != null ? world.getRegistryManager() : null;
    }
    ...
    private EditorRow addStringField(String label, String startVal, String placeholder,
        ...
        field.setMaxLength(128); field.setText(startVal != null ? startVal : "");
        if (placeholder != null) field.setPlaceholder(Text.literal(placeholder).formatted(net.minecraft.util.Formatting.DARK_GRAY));
        field.setChangedListener(text -> { hasUnsavedChanges = true; action.accept(text); });
        EditorRow row = new EditorRow(label, RowType.STRING, field);
        row.suggestions = suggestions;
        this.rows.add(row);
        return row;
    ...
    private static List<String> scanResourceShortNames(String rootFolder, java.util.function.Predicate<net.minecraft.util.Identifier> filter, String stripSuffix) {
        net.minecraft.resource.ResourceManager rm = MinecraftClient.getInstance().getResourceManager();
        java.util.LinkedHashSet<String> out = new java.util.LinkedHashSet<>();
        for (net.minecraft.util.Identifier resId : rm.findResources(rootFolder, filter).keySet()) {
    ...
    private void renderAutocompleteDropdown(DrawContext c, int mouseX, int mouseY) {
        ...
        c.drawBorder(openDropdownX - 1, openDropdownY - 1, openDropdownW + 2, boxH + 2, 0xFFFFAA00);
        ...
        for (int i = 0; i < openDropdownFiltered.size(); i++) {
            ...
            c.drawTextWithShadow(parent.getTextRenderer(), openDropdownFiltered.get(i), openDropdownX + 2, itemY + 2, hovered ? 0xFFFFFFFF : 0xFFCCCCCC);
            c.disableScissor();
        }
    ...
    private boolean handleAutocompleteClick(double mx, double my) {
        ...
        if (index >= 0 && index < openDropdownFiltered.size()) {
            ...
            openDropdownField.setText(openDropdownFiltered.get(index));
    ...
    public void render(DrawContext c, int mouseX, int mouseY, float delta, int x, int y, int width, int height) {
        ...
        if (currentState == State.LIST) {
            ...
            c.disableScissor();
            c.drawTextWithShadow(parent.getTextRenderer(), isTypeDropdownOpen ? "▲" : "▼", typeBoxX + typeFilterW - 10, searchY + 4, 0xFFAAAAAA);
            ...
            if (isTypeDropdownOpen) {
                ...
                c.drawBorder(typeBoxX, searchY + 16, typeFilterW, listBoxH, 0xFFFFAA00);
            ...
            for (int i = 0; i < ids.size(); i++) {
                ...
                c.drawBorder(itemX, itemY, itemSize, itemSize, hovered ? 0xFFFFAA00 : (isArmor ? 0xFF3399FF : 0xFF444444));
                ...
                if (isArmor && data != null && data.realItemId != null) {
                    ...
                    net.minecraft.item.Item realItem = realId != null ? net.minecraft.registry.Registries.ITEM.get(realId) : Items.AIR;
                ...
                if (renderStack != null) {
                    c.getMatrices().push();
                    c.getMatrices().translate(itemX + (itemSize/2f) - 8, itemY + (itemSize/2f) - 8, 0);
                ...
                java.util.List<Text> nameLines = data != null ? data.getFormattedNameLines(registries())
                        : java.util.List.of(Text.literal("§d" + ids.get(i)));
                int maxW = itemSize - 4;
            ...
            c.drawTextWithShadow(parent.getTextRenderer(), "X", x + width - 40, topY + 2, hovDelete ? 0xFF5555 : 0xAAAAAA);

            Text title = editingData != null ? editingData.getFormattedNameText(registries()) : Text.literal("§d" + editingId.toUpperCase());
            ...
            for (int i = 0; i < this.rows.size(); i++) {
                ...
                c.drawTextWithShadow(parent.getTextRenderer(), navLabel, x + 6, navY + 3, isCurrentSection ? 0xFFFFAA00 : 0xFFCCCCCC);
                c.disableScissor();
                ...
                if (this.rows.get(i).label.contains("MODELOS 3D")) {
                    for (int p = 0; p < outlinerParts.size(); p++) {
                        ...
                        c.drawTextWithShadow(parent.getTextRenderer(), "• Part " + p, x + 13, navY + 2, isSelectedPart ? 0xFF00FFAA : 0xFFAAAAAA);
                        navY += 15;
                    }
                }
            }
            ...
            for (int i = 0; i < this.rows.size(); i++) {
                ...
                if (row.type != RowType.BUTTON) {
                    c.drawTextWithShadow(parent.getTextRenderer(), "§f" + row.label, propsX + 15, rowY, 0xFFFFFF);
                }
                ...
                if (row.type == RowType.STRING || row.type == RowType.INT || row.type == RowType.DOUBLE) {
                    ...
                    if (row.suggestions != null && row.textField.isFocused()) {
                        ...
                        this.openDropdownFiltered = filterSuggestions(row.suggestions.get(), row.textField.getText());
                    }
                }
                else if (row.type == RowType.TOGGLE) {
    ...
    private void renderConfirmPopup(DrawContext c, int mx, int my) {
        ...
        c.drawCenteredTextWithShadow(parent.getTextRenderer(), "§c§l" + confirmPopupTitle, vWidth / 2, py + 15, 0xFFFFFF);
        c.drawCenteredTextWithShadow(parent.getTextRenderer(), confirmPopupMessage, vWidth / 2, py + 32, 0xAAAAAA);
