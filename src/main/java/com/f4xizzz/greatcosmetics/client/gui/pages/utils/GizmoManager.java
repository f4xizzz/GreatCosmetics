...
public class GizmoManager {
    ...
    public static final RenderLayer GIZMO_LINES_NO_DEPTH = new RenderLayer.MultiPhase(
            ...
            RenderLayer.MultiPhaseParameters.builder()
                    ...
                    .depthTest(new RenderPhase.DepthTest("greatcosmetics_always_depth", org.lwjgl.opengl.GL11.GL_ALWAYS))
    ...
    public static void updateScreenProjection(Matrix4f modelViewPosition) {
        ...
        org.joml.Quaternionf viewRotation = new org.joml.Quaternionf(
                MinecraftClient.getInstance().getEntityRenderDispatcher().getRotation()).invert();
    ...
    private static Vector2f projectToScreen(Matrix4f mvp, float x, float y, float z) {
        ...
        float px = (ndcX * 0.5f + 0.5f) * window.getFramebufferWidth();
        float py = (1.0f - (ndcY * 0.5f + 0.5f)) * window.getFramebufferHeight();
    ...
    private static Vector2f virtualMouseToFramebuffer(double virtualX, double virtualY) {
        var window = MinecraftClient.getInstance().getWindow();
        float scaleMultiplier = (window.getFramebufferHeight() / 450f) / (float) window.getScaleFactor();
        ...
        double realX = virtualX * scaleMultiplier * window.getScaleFactor();
        double realY = virtualY * scaleMultiplier * window.getScaleFactor();
    ...
    public static void handleDrag(double virtualMouseX, double virtualMouseY, boolean isSneaking) {
        ...
        boolean allAxes = net.minecraft.client.gui.screen.Screen.hasShiftDown();

        if (currentMode == Mode.ROTATE) {
            float angle = computeScreenAngle(currentAxis, virtualMouseX, virtualMouseY);
            if (!Float.isNaN(angle)) {
                float deltaDeg = MathHelper.wrapDegrees(angle - lastAngle);
                ...
                deltaDeg = MathHelper.clamp(deltaDeg, -90f, 90f);
                if (!isSneaking) {
