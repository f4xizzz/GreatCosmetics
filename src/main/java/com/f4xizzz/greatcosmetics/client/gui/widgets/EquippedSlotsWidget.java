...
public class EquippedSlotsWidget {
    ...
    public void render(DrawContext c, int vMouseX, int vMouseY, int vWidth, int vHeight, boolean isDevTabActive) {
        if (isDevTabActive || MinecraftClient.getInstance().player == null) {
            animProgress = 0.0f;
            isOpen = false;
            return;
        }
                ...
                if (boxX + boxSize > currentX && boxX < vWidth) {
                    ...
                    if (j < eqInSlot.size()) {
                        ...
                        if (equippedData.realItemId != null) {
                            ...
                            net.minecraft.util.Identifier realId = net.minecraft.util.Identifier.tryParse(equippedData.realItemId);
                            net.minecraft.item.Item realItem = realId != null ? net.minecraft.registry.Registries.ITEM.get(realId) : Items.CARVED_PUMPKIN;
                            renderStack = new ItemStack(realItem);
                        } else {
                            renderStack = new ItemStack(Items.CARVED_PUMPKIN);
                            renderStack.set(DataComponentTypes.CUSTOM_MODEL_DATA, new CustomModelDataComponent(equippedData.cmd));
                        }

                        c.getMatrices().push();
                        c.getMatrices().translate(boxX + 3.5f, rowY + 3.5f, 0);
                        c.getMatrices().scale(1.2f, 1.2f, 1.0f);
                        ...
                        if (over(vMouseX, vMouseY, boxX, rowY, boxSize, boxSize)) {
                            c.drawBorder(boxX, rowY, boxSize, boxSize, 0x44FFFFFF);
                        }
                    } else {
                        c.drawBorder(boxX, rowY, boxSize, boxSize, ClientCosmeticCache.isDevModeActive ? 0x44FFAA00 : 0x22FFFFFF);
                        Identifier icon = SLOT_ICONS.get(slotName);
                        if (icon != null) {
                            int iconSize = 16;
                            c.getMatrices().push();
                            c.getMatrices().translate(boxX + (boxSize - iconSize) / 2f, rowY + (boxSize - iconSize) / 2f, 0);
                            c.drawTexture(icon, 0, 0, 0, 0, iconSize, iconSize, iconSize, iconSize);
                            c.getMatrices().pop();
                        } else {
                            String shortName = slotName.length() >= 3 ? slotName.substring(0, 3) : slotName;
                            c.drawCenteredTextWithShadow(textRenderer, shortName, boxX + (boxSize/2), rowY + (boxSize/2) - 4, ClientCosmeticCache.isDevModeActive ? 0xFFFFAA00 : 0xFF444444);
                        }
                    }
                }
            }
        ...
        // ==========================================
        if (hasDevPermission(MinecraftClient.getInstance().player)) { // <-- Alterado para o novo método
            int btnW = 60;
            int btnH = 12;
            ...
            c.drawCenteredTextWithShadow(textRenderer, devText, 0, 0, 0xFFFFFF);
            c.getMatrices().pop();
        }
    }

    private void drawHandle(DrawContext c, int vMouseX, int vMouseY, int currentX, int drawerY, int drawerHeight) {
        int handleW = 14;
        int handleH = 40;
        ...
        c.drawCenteredTextWithShadow(parent.getTextRenderer(), isOpen ? ">" : "<", handleX + handleW/2, handleY + (handleH/2) - 4, hovered ? 0xFFFFAA00 : 0xFFAAAAAA);
    }

    public boolean mouseClicked(int vMouseX, int vMouseY, int vWidth, int vHeight, boolean isDevTabActive) {
        if (isDevTabActive || MinecraftClient.getInstance().player == null) return false;
    ...
    public boolean mouseScrolled(double vMx, double vMy, double hAmount, double vAmount) {
        if (isOpen && maxScrollX > 0) {
            ...
            int currentX = MinecraftClient.getInstance().getWindow().getScaledWidth() - dynamicDrawerWidth;
            if (vMx >= currentX) {
    ...
    private void toggleAccessoryVisibility(String cosmeticId, ClientCosmeticCache.PlayerSettings settings, UUID uuid) {
        ...
        ClientPlayNetworking.send(new com.f4xizzz.greatcosmetics.network.ToggleCosmeticVisibilityPayload(cosmeticId, newState));
