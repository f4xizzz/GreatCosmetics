...
public class AcessoriesPage extends WardrobePage {
    ...
    private Text formatEffectForTooltip(String eff) {
        ...
        Text name = Text.literal(eff.toUpperCase());
        if (id != null) {
            var entry = net.minecraft.registry.Registries.STATUS_EFFECT.getEntry(id);
            if (entry.isPresent()) name = entry.get().value().getName();
        }
        return Text.literal("").append(name).append(Text.literal(" " + level));
    }
    ...
    public void render(DrawContext c, int mouseX, int mouseY, float delta, int x, int y, int width, int height) {
        ...
        if (MinecraftClient.getInstance().player != null) {
            equippedIds = ClientCosmeticCache.getEquipped(MinecraftClient.getInstance().player.getUuid());
        }
        ...
        c.drawBorder(btnClearX, filterY, btnClearW, btnClearH, hoverClear ? 0xFFFFAA00 : 0xFF222222);
        c.drawCenteredTextWithShadow(getTextRenderer(), "X", btnClearX + (btnClearW/2), filterY + 4, 0xFFFFFF);
        ...
        for (CosmeticData data : items) {
            ...
            if (isEquipped) {
                ...
                c.drawBorder(itemX, itemY, iconRealSize, iconRealSize, 0xFF00FF00);
            }
            ...
            c.getMatrices().push();
            c.getMatrices().translate(itemX, itemY, 0);
            ...
            if (isFavorite || hoveringThisItem) {
                ...
                c.drawTextWithShadow(getTextRenderer(), isFavorite ? "★" : "☆", starX, starY,
            ...
            if (hoveringThisItem) {
                ...
                List<Text> safeNameLines = data.getFormattedNameLines(regs);
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (over(mouseX, mouseY, btnClearX, filterY, btnClearW, btnClearH)) {
            playClick();
            ClientPlayNetworking.send(new ClearAllCosmeticsPayload());
            return true;
        }
        ...
        if (isDropdownOpen) {
            for (int i = 0; i < categoryIds.length; i++) {
                ...
                if (over(mouseX, mouseY, panelX + 12, itemY, filterBoxW, 14)) {
                    ...
                    if (MinecraftClient.getInstance().player != null) {
                        float eyeY = MinecraftClient.getInstance().player.getStandingEyeHeight();
