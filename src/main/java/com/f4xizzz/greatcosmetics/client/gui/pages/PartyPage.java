...
public class PartyPage extends WardrobePage {
    ...
    private void applyPreviewPose() {
        ...
        try {
            ...
            previewEntity.getDataTracker().set(PokemonEntity.getPOSE_TYPE(), pose);
            previewEntity.getDataTracker().set(PokemonEntity.getBATTLE_ID(),
    ...
    /** Cobblemon 1.8: {@code Pokemon.initialize()} (chamado por {@code PokemonProperties.create()})
     *  passou a gerar um moveset via {@code CobblemonMovesetBuilders} — um JsonDataRegistry de
     *  datapack. No CLIENT esse mapa fica VAZIO (não sincroniza como o resto), então todo
     *  {@code create()} de dummy pra render explodia com
     *  {@code IllegalArgumentException: Unknown MovesetBuilder id: cobblemon:wild}. Registra um
     *  {@code DefaultMovesetBuilder} vazio pros ids que o Cobblemon usa por padrão (wild/alpha),
     *  só client-side, sem sobrescrever nada que por acaso venha a existir de verdade. */
    private static void ensureClientMovesetBuilders() {
        try {
            java.util.Map<net.minecraft.util.Identifier, com.cobblemon.mod.common.api.moves.MovesetBuilder> map =
                    com.cobblemon.mod.common.CobblemonMovesetBuilders.INSTANCE.getMovesetBuilders();
            for (String path : new String[]{"wild", "alpha"}) {
                net.minecraft.util.Identifier id = net.minecraft.util.Identifier.of("cobblemon", path);
    ...
    private void rebuildPreviewEntity() {
        ...
        try {
            ...
            PokemonEntity entity = (PokemonEntity) CobblemonEntities.POKEMON.create(client.world);
            ...
            entity.onTrackedDataSet(PokemonEntity.getASPECTS());
            entity.onTrackedDataSet(PokemonEntity.getSPECIES());
            entity.onTrackedDataSet(PokemonEntity.getSCALE_MODIFIER());
    ...
    private boolean hasDevPermission(net.minecraft.client.network.ClientPlayerEntity player) {
    ...
    private void updateAvailableSkins() {
        ...
        if (client.getWindow() != null) {
            float frameScale = client.getWindow().getFramebufferHeight() / 450f;
            skinsScale = 0.8f * (frameScale / (float) client.getWindow().getScaleFactor());
        }
            ...
            for (int i = 0; i < partySize && i < 6; i++) {
                ...
                if (pokemon != null) {
                    ...
                    if (entity != null) {
                        ...
                        entity.getDataTracker().set(PokemonEntity.getSPECIES(), pokemon.getSpecies().getResourceIdentifier().toString());
                        entity.getDataTracker().set(PokemonEntity.getASPECTS(), new HashSet<>(pokemon.getAspects()));
                        entity.getDataTracker().set(PokemonEntity.getSCALE_MODIFIER(), pokemon.getScaleModifier());
    ...
    public void render(DrawContext c, int mouseX, int mouseY, float delta, int x, int y, int width, int height) {
        ...
        if (org.lwjgl.glfw.GLFW.glfwGetMouseButton(MinecraftClient.getInstance().getWindow().getHandle(), org.lwjgl.glfw.GLFW.GLFW_MOUSE_BUTTON_LEFT) != org.lwjgl.glfw.GLFW.GLFW_PRESS) {
            isDraggingSkinScrollbar = false;
        }

        // NOVO: Dá 600ms pro servidor processar a skin e força a tela a se recarregar sozinha!
        if (pendingRefreshTime > 0 && Util.getMeasuringTimeMs() >= pendingRefreshTime) {
        ...
        net.minecraft.registry.RegistryWrapper.WrapperLookup regs = client.world != null ? client.world.getRegistryManager() : null;
        ...
        if (currentAvailableSkins.isEmpty()) {
            ...
            for (SkinDisplayRow row : skinDisplayRows) {
                ...
                if (row.isHeader) {
                    ...
                    c.drawTextWithShadow(parent.getTextRenderer(), "§l" + row.headerLabel, lastListX + 6, itemY + (itemHeight - 8) / 2, groupColor);
                    continue;
                }
                ...
                c.drawBorder(lastListX, itemY, lastListW, itemHeight, borderColor);

                ModelWidget widget = skinWidgets.get(skin.getId());
                if (widget != null) {
                    c.getMatrices().push();
                    float frameScale = client.getWindow().getFramebufferHeight() / 450f;
                    float sm = frameScale / (float) client.getWindow().getScaleFactor();

                    c.getMatrices().scale(1.0f / sm, 1.0f / sm, 1.0f);
                ...
                net.minecraft.text.Text displayName = regs != null
                        ? com.f4xizzz.greatcosmetics.util.BackpackManager.parseMiniMessage("§f" + skin.getDisplayName(), regs)
                        : net.minecraft.text.Text.literal("§f" + skin.getDisplayName());
                ...
                if (!isUnlocked) {
                    ...
                    c.drawTextWithShadow(parent.getTextRenderer(), "🔒", lastListX + lastListW - 16, itemY + 4, 0xFFCCCCCC);
                    ...
                    c.drawBorder(previewBtnX, previewBtnY, previewBtnSize, previewBtnSize, isPreviewingThis ? 0xFF55FF55 : 0xFFAAAAAA);
                    c.drawTextWithShadow(parent.getTextRenderer(), "👁", previewBtnX + 3, previewBtnY + 3, 0xFFFFFFFF);
                }
            }
        ...
        float scaleMultiplier = targetScale / (float) client.getWindow().getScaleFactor();
        ...
        if (previewingSkin != null) {
            net.minecraft.text.Text skinName = regs != null
        ...
        if (previewingSkin == null && getValidPokemonCount() > 1) {
            ...
            c.drawBorder(leftArrowX, arrowY, arrowW, arrowH, hovLeft ? 0xFFFFAA00 : 0x22FFFFFF);
            c.drawCenteredTextWithShadow(parent.getTextRenderer(), "<", leftArrowX + (arrowW / 2), arrowY + (arrowH / 2) - 4, hovLeft ? 0xFFFFAA00 : 0xFFAAAAAA);

            c.fill(rightArrowX, arrowY, rightArrowX + arrowW, arrowY + arrowH, hovRight ? 0x66222222 : 0x330A0A0A);
            c.drawBorder(rightArrowX, arrowY, arrowW, arrowH, hovRight ? 0xFFFFAA00 : 0x22FFFFFF);
            c.drawCenteredTextWithShadow(parent.getTextRenderer(), ">", rightArrowX + (arrowW / 2), arrowY + (arrowH / 2) - 4, hovRight ? 0xFFFFAA00 : 0xFFAAAAAA);
        }
        ...
        for (int i = 0; i < 6; i++) {
            ...
            c.drawBorder(slotX, startY, slotSize, slotSize, borderColor);
            ...
            if (partyCache[i] != null) {
                ...
                c.getMatrices().push();
                c.getMatrices().scale(1.0f / scaleMultiplier, 1.0f / scaleMultiplier, 1.0f);
    ...
    public boolean mouseClicked(double mx, double my, int button) {
        ...
        if (hasDevPermission(MinecraftClient.getInstance().player)) {
            int devBtnW = 40;
        ...
        if (com.f4xizzz.greatcosmetics.client.ClientCosmeticCache.isDevModeActive) {
            if (over(mx, my, btnClearCurX, btnClearCurY, btnClearW, btnClearH)) {
                playClick();
                ClientPlayNetworking.send(new ClearPokemonSkinsPayload(this.selectedSlot));
                pendingRefreshTime = Util.getMeasuringTimeMs() + 600; // Agenda a atualização
                return true;
            }
            if (over(mx, my, btnClearAllX, btnClearCurY, btnClearW, btnClearH)) {
                playClick();
                ClientPlayNetworking.send(new ClearPokemonSkinsPayload(-1));
                pendingRefreshTime = Util.getMeasuringTimeMs() + 600; // Agenda a atualização
                return true;
            }
        }
        ...
        if (!currentAvailableSkins.isEmpty() && over(mx, my, lastListX, lastListY, lastListW, lastListH)) {
            for (SkinDisplayRow row : skinDisplayRows) {
                ...
                if (over(mx, my, lastListX, itemY, lastListW, itemHeight)) {
                    ...
                    if (!isCompatible) {
                        try { MinecraftClient.getInstance().getSoundManager().play(net.minecraft.client.sound.PositionedSoundInstance.master(net.minecraft.sound.SoundEvents.BLOCK_NOTE_BLOCK_BASS.value(), 1.0F)); } catch (Exception ignored) {}
                        return true;
                    }
                    ...
                    playClick();
                    ClientPlayNetworking.send(new EquipPokemonSkinPayload(this.selectedSlot, skin.getId()));
                    pendingRefreshTime = Util.getMeasuringTimeMs() + 600; // Agenda a atualização
                    return true;
                }
            }
    ...
    private void drawMarqueeText(DrawContext c, net.minecraft.text.Text text, int x, int y, int maxWidth, int clipY, int clipH) {
        int textWidth = parent.getTextRenderer().getWidth(text);

        if (textWidth <= maxWidth) {
            c.drawTextWithShadow(parent.getTextRenderer(), text, x, y, 0xFFFFFF);
            return;
        }
        ...
        if (clipY1 < clipY2) {
            ...
            c.drawTextWithShadow(parent.getTextRenderer(), text, x - offset, y, 0xFFFFFF);
            c.disableScissor();
        }
    }
