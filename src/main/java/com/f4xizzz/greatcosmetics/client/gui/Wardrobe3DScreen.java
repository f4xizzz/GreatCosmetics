...
public class Wardrobe3DScreen extends Screen {
    ...
    public void enablePerfectScissor(DrawContext c, int x, int y, int width, int height) {
        net.minecraft.client.MinecraftClient client = net.minecraft.client.MinecraftClient.getInstance();
        double scale = client.getWindow().getScaleFactor();
        org.joml.Matrix4f matrix = c.getMatrices().peek().getPositionMatrix();
        ...
        int screenY = (int) Math.round(client.getWindow().getFramebufferHeight() - end.y() * scale);
     ...
     */
    public void enableScissorStacked(DrawContext c, int x, int y, int width, int height) {
        org.joml.Matrix4f matrix = c.getMatrices().peek().getPositionMatrix();
        org.joml.Vector4f start = new org.joml.Vector4f((float) x, (float) y, 0.0F, 1.0F).mul(matrix);
        org.joml.Vector4f end = new org.joml.Vector4f((float) (x + width), (float) (y + height), 0.0F, 1.0F).mul(matrix);
    ...
    protected void init() {
        if (this.client != null && this.client.player != null) {
            if (!this.isInitialized) {
                ...
                this.oldFov = this.client.options.getFov().getValue();

                this.fullWAcc = this.textRenderer.getWidth(Tab.ACESSORIES.getDisplayName()) + 16;
                this.fullWCus = this.textRenderer.getWidth(Tab.CUSTOMIZE.getDisplayName()) + 16;
                this.fullWPrt = this.textRenderer.getWidth(Tab.PARTY.getDisplayName()) + 16;
                this.fullWTag = this.textRenderer.getWidth(Tab.TAGS.getDisplayName()) + 16;
                this.fullWDev = this.textRenderer.getWidth(Tab.DEV.getDisplayName()) + 16;
            ...
            this.client.options.getFov().setValue(45);
    ...
    private void resetCameraToNeutral() {
        if (this.client != null && this.client.player != null) {
            ...
            targetPitch = this.hasBackground ? this.client.player.getPitch() : 0f;
            targetDistance = 3.5;
            targetFocusY = this.client.player.getStandingEyeHeight() - 0.5f;
            targetPan = 0.0f;
            isZoomLocked = false;
        }
    }
    ...
    public void render(DrawContext c, int mouseX, int mouseY, float delta) {
        ...
        this.client.options.sneakKey.setPressed(shouldSneak);

        this.client.player.setYaw(this.lockedPlayerYaw);
        this.client.player.prevYaw = this.lockedPlayerYaw;
        this.client.player.setBodyYaw(this.lockedPlayerYaw);
        this.client.player.prevBodyYaw = this.lockedPlayerYaw;
        this.client.player.setHeadYaw(this.lockedPlayerYaw);
        this.client.player.prevHeadYaw = this.lockedPlayerYaw;
        this.client.player.setPitch(0f);
        this.client.player.prevPitch = 0f;
        ...
        currentYaw = MathHelper.lerpAngleDegrees(animSpeed, currentYaw, targetYaw);

        float targetScale = this.client.getWindow().getFramebufferHeight() / 450f;
        float scaleMultiplier = targetScale / (float) this.client.getWindow().getScaleFactor();

        int vMouseX = (int) (mouseX / scaleMultiplier);
        int vMouseY = (int) (mouseY / scaleMultiplier);
        int vWidth = (int) (this.client.getWindow().getFramebufferWidth() / targetScale);
        int vHeight = (int) (this.client.getWindow().getFramebufferHeight() / targetScale);

        c.getMatrices().push();
        c.getMatrices().scale(scaleMultiplier, scaleMultiplier, 1.0f);
    ...
    private void renderGizmoSidebar(DrawContext c, int mouseX, int mouseY) {
        ...
        if (showGeoOffsetSection()) {
            ...
            c.drawCenteredTextWithShadow(this.textRenderer, "-", SIDEBAR_X + 2 + btnW / 2, rowY + 3, 0xFFFFFFFF);
    ...
    private void drawTechTab(DrawContext c, Tab tab, String letter, String fullName, int x, int y, int currentW, boolean isHovered) {
        ...
        if (currentW < tabBaseW + 10) {
            c.drawCenteredTextWithShadow(this.textRenderer, letter, x + (tabBaseW / 2), y + (tabH / 2) - 4, textColor);
        } else {
            c.drawTextWithShadow(this.textRenderer, fullName, x + 8, y + (tabH / 2) - 4, textColor);
        }
    ...
    public boolean mouseDragged(double mx, double my, int button, double deltaX, double deltaY) {
        if (this.client == null || this.client.getWindow() == null) return super.mouseDragged(mx, my, button, deltaX, deltaY);
        ...
        if (button == GLFW.GLFW_MOUSE_BUTTON_LEFT) {
            ...
            targetPitch = MathHelper.clamp(targetPitch + (float) deltaY * 1.5f, -90f, 90f);
            return true;
        }
    ...
    public boolean mouseScrolled(double mx, double my, double horizontalAmount, double verticalAmount) {
        if (this.client == null || this.client.getWindow() == null) return super.mouseScrolled(mx, my, horizontalAmount, verticalAmount);
        ...
        targetDistance = MathHelper.clamp(targetDistance, 0.5, 7.0);
        return true;
    }
    ...
    public void removed() {
        ...
        if (this.client != null) {
            ...
            this.client.options.getFov().setValue(this.oldFov);
